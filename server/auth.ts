...
export async function loginWithCode(email: string, code: string): Promise<{ user: User; token: string } | null> {
  try {
    const result = await db.select().from(verificationCodes).where(eq(verificationCodes.email, email)).orderBy(desc(verificationCodes.createdAt)).limit(1);
    ...
    await db.update(verificationCodes).set({ verified: true }).where(eq(verificationCodes.id, verificationCode.id));
