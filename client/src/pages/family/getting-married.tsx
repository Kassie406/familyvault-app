...
import { Plus, Search, ChevronRight, HeartHandshake, Calendar, CreditCard, Users, FileText, MapPin, Camera, MoreHorizontal, Check, X, HelpCircle, Bell } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { useLocation } from "wouter";
import { LuxuryCard } from '@/components/luxury-cards';

type SectionKey = "planning" | "legal" | "financial" | "guests" | "timeline" | "vendors";
...
export default function GettingMarried() {
  ...
  const [pageTitle, setPageTitle] = useState('Getting Married');
  const [tempTitle, setTempTitle] = useState('Getting Married');
  ...
  const [sections] = useState<Section[]>([
    {
      ...
      desc: "Vendor contacts, venue details, timeline & planning documents",
      icon: <Calendar className="h-4 w-4" />,
      count: 0,
      href: "/family/wedding/planning",
    },
    {
      key: "legal",
      title: "Legal Documents",
      desc: "Marriage license, name change documents & legal paperwork",
      icon: <FileText className="h-4 w-4" />,
      count: 0,
      href: "/family/wedding/legal",
    },
    {
      key: "financial",
      title: "Financial Planning",
      desc: "Budget tracking, vendor payments & financial documents",
      icon: <CreditCard className="h-4 w-4" />,
      count: 0,
      href: "/family/wedding/financial",
    },
    {
      key: "guests",
      title: "Guest Management",
      desc: "Guest lists, RSVP tracking & contact information",
      icon: <Users className="h-4 w-4" />,
      count: 0,
      href: "/family/wedding/guests",
    },
    {
      key: "timeline",
      title: "Timeline & Schedule",
      desc: "Wedding day timeline, appointments & key milestones",
      icon: <MapPin className="h-4 w-4" />,
      count: 0,
      href: "/family/wedding/timeline",
    },
    {
      key: "vendors",
      title: "Vendor Management",
      desc: "Photographer, caterer, florist & all service providers",
      icon: <Camera className="h-4 w-4" />,
      count: 0,
      href: "/family/wedding/vendors",
    },
  ]);
  ...
  const handleDocumentClick = (ev: MouseEvent) => {
    ...
    const clickedMenu = document.getElementById("wedding-add-menu")?.contains(target);
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search wedding information…"
                value={query}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button
              ...
              aria-controls="wedding-add-menu"
              data-testid="button-add-wedding"
            >
              + Add Wedding Planning
            </button>
            ...
            {addOpen && (
              ...
              >
                <div className="px-3 py-2 text-xs text-white/50">Add to Wedding Planning</div>
                ...
                <MenuItem onClick={() => setAddOpen(false)}>Budget Item</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Guest</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Timeline Event</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Vendor Contact</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Wedding Expense</MenuItem>
                <div className="px-3 py-2 text-xs text-white/50">Import</div>
                <MenuItem onClick={() => setAddOpen(false)}>Import guest list…</MenuItem>
      ...
      <div className="mx-auto max-w-6xl px-6 py-8">
        <div className="mb-6">
          <p className="text-white/60 text-sm">
            Keep all your wedding planning organized with vendor information, timelines, and important documents.
          </p>
        </div>
        ...
        {totalItems === 0 && filtered.length > 0 && (
          <div className="mt-12 text-center" data-testid="getting-started">
            <HeartHandshake className="h-16 w-16 text-white/40 mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-white mb-2">Plan Your Perfect Wedding</h3>
