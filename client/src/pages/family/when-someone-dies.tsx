...
import { Plus, Search, ChevronRight, Leaf, FileText, Phone, DollarSign, Heart, Scale, MoreHorizontal, Check, X, HelpCircle, Bell } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { useLocation } from "wouter";
import { LuxuryCard } from '@/components/luxury-cards';

type SectionKey = "immediate" | "financial" | "legal" | "support" | "notifications" | "memorial";
...
export default function WhenSomeoneDies() {
  ...
  const [pageTitle, setPageTitle] = useState('When Someone Dies');
  const [tempTitle, setTempTitle] = useState('When Someone Dies');
  ...
  const [sections] = useState<Section[]>([
    {
      ...
      desc: "Death certificates, funeral arrangements & immediate notifications",
      icon: <FileText className="h-4 w-4" />,
      count: 0,
      href: "/family/bereavement/immediate",
    },
    {
      key: "financial",
      title: "Financial Matters",
      desc: "Estate settlement, accounts, insurance claims & financial obligations",
      icon: <DollarSign className="h-4 w-4" />,
      count: 0,
      href: "/family/bereavement/financial",
    },
    {
      key: "legal",
      title: "Legal Procedures",
      desc: "Probate process, will execution & legal documentation",
      icon: <Scale className="h-4 w-4" />,
      count: 0,
      href: "/family/bereavement/legal",
    },
    {
      key: "support",
      title: "Support Resources",
      desc: "Grief counseling, support groups & memorial planning",
      icon: <Heart className="h-4 w-4" />,
      count: 0,
      href: "/family/bereavement/support",
    },
    {
      key: "notifications",
      title: "Notifications",
      desc: "Contact lists, employer notifications & service cancellations",
      icon: <Phone className="h-4 w-4" />,
      count: 0,
      href: "/family/bereavement/notifications",
    },
    {
      key: "memorial",
      title: "Memorial & Legacy",
      desc: "Memorial services, obituaries & remembrance planning",
      icon: <Leaf className="h-4 w-4" />,
      count: 0,
      href: "/family/bereavement/memorial",
    },
  ]);
  ...
  const handleDocumentClick = (ev: MouseEvent) => {
    ...
    const clickedMenu = document.getElementById("bereavement-add-menu")?.contains(target);
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            ...
            {addOpen && (
              ...
              >
                <div className="px-3 py-2 text-xs text-white/50">Add to Bereavement Support</div>
                <MenuItem icon={<FileText className="h-4 w-4" />} onClick={() => setAddOpen(false)}>
                  Immediate Task
                </MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Financial Record</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Legal Document</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Support Resource</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Notification Task</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Memorial Planning</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Important Contact</MenuItem>
      ...
      <div className="mx-auto max-w-6xl px-6 py-8">
        <div className="mb-6">
          <p className="text-white/60 text-sm">
            Access organized guidance and resources to help manage necessary tasks during bereavement.
          </p>
        </div>
        ...
        {totalItems === 0 && filtered.length > 0 && (
          <div className="mt-12 text-center" data-testid="getting-started">
            <Leaf className="h-16 w-16 text-white/40 mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-white mb-2">Navigate Difficult Times</h3>
