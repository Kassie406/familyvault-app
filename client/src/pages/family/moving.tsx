...
import { Plus, Search, ChevronRight, Package, MapPin, Truck, CheckSquare, FileText, CreditCard, MoreHorizontal, Check, X, HelpCircle, Bell } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { useLocation } from "wouter";
import { LuxuryCard } from '@/components/luxury-cards';

type SectionKey = "checklist" | "companies" | "addresses" | "documents" | "finances" | "inventory";
...
export default function Moving() {
  ...
  const [pageTitle, setPageTitle] = useState('Moving');
  const [tempTitle, setTempTitle] = useState('Moving');
  ...
  const [sections] = useState<Section[]>([
    {
      ...
      desc: "Timeline, tasks & moving day preparations",
      icon: <CheckSquare className="h-4 w-4" />,
      count: 0,
      href: "/family/moving/checklist",
    },
    {
      key: "companies",
      title: "Moving Companies",
      desc: "Quotes, contracts & moving service provider information",
      icon: <Truck className="h-4 w-4" />,
      count: 0,
      href: "/family/moving/companies",
    },
    {
      key: "addresses",
      title: "Address Changes",
      desc: "Utilities, subscriptions & address update documentation",
      icon: <MapPin className="h-4 w-4" />,
      count: 0,
      href: "/family/moving/addresses",
    },
    {
      key: "documents",
      title: "Important Documents",
      desc: "Lease agreements, closing documents & legal paperwork",
      icon: <FileText className="h-4 w-4" />,
      count: 0,
      href: "/family/moving/documents",
    },
    {
      key: "finances",
      title: "Moving Finances",
      desc: "Budget, expenses, deposits & financial planning",
      icon: <CreditCard className="h-4 w-4" />,
      count: 0,
      href: "/family/moving/finances",
    },
    {
      key: "inventory",
      title: "Inventory & Packing",
      desc: "Item lists, packing schedules & moving inventory",
      icon: <Package className="h-4 w-4" />,
      count: 0,
      href: "/family/moving/inventory",
    },
  ]);
  ...
  const handleDocumentClick = (ev: MouseEvent) => {
    ...
    const clickedMenu = document.getElementById("moving-add-menu")?.contains(target);
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button
              ...
              aria-controls="moving-add-menu"
              data-testid="button-add-moving"
            >
              + Add Moving Item
            </button>
            ...
            {addOpen && (
              ...
              >
                <div className="px-3 py-2 text-xs text-white/50">Add to Moving Plan</div>
                <MenuItem icon={<CheckSquare className="h-4 w-4" />} onClick={() => setAddOpen(false)}>
                  Checklist Item
                </MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Moving Company</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Address Change</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Important Document</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Moving Expense</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Inventory Item</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Packing List</MenuItem>
                <div className="px-3 py-2 text-xs text-white/50">Import</div>
                <MenuItem onClick={() => setAddOpen(false)}>Import moving list…</MenuItem>
      ...
      <div className="mx-auto max-w-6xl px-6 py-8">
        ...
        {totalItems === 0 && filtered.length > 0 && (
          <div className="mt-12 text-center" data-testid="getting-started">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">Organize Your Move</h3>
