...
import { Plus, Search, ChevronRight, Home as HouseIcon, DollarSign, FileText, Key, MapPin, Calculator, MoreHorizontal, Check, X, HelpCircle, Bell } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { useLocation } from "wouter";
import { LuxuryCard } from '@/components/luxury-cards';

type SectionKey = "financial" | "property" | "legal" | "professionals" | "inspections" | "closing";
...
export default function HomeBuying() {
  ...
  const [pageTitle, setPageTitle] = useState('Home Buying');
  const [tempTitle, setTempTitle] = useState('Home Buying');
  ...
  const [sections] = useState<Section[]>([
    {
      ...
      desc: "Pre-approval letters, budget planning & mortgage information",
      icon: <DollarSign className="h-4 w-4" />,
      count: 0,
      href: "/family/home-buying/financial",
    },
    {
      key: "property",
      title: "Property Research",
      desc: "Property listings, neighborhood information & market analysis",
      icon: <MapPin className="h-4 w-4" />,
      count: 0,
      href: "/family/home-buying/property",
    },
    {
      key: "legal",
      title: "Legal Documents",
      desc: "Contracts, inspection reports & closing documentation",
      icon: <FileText className="h-4 w-4" />,
      count: 0,
      href: "/family/home-buying/legal",
    },
    {
      key: "professionals",
      title: "Professional Contacts",
      desc: "Real estate agents, lenders, inspectors & service providers",
      icon: <Key className="h-4 w-4" />,
      count: 0,
      href: "/family/home-buying/professionals",
    },
    {
      key: "inspections",
      title: "Inspections & Appraisals",
      desc: "Home inspections, appraisals & property evaluations",
      icon: <HouseIcon className="h-4 w-4" />,
      count: 0,
      href: "/family/home-buying/inspections",
    },
    {
      key: "closing",
      title: "Closing Process",
      desc: "Closing costs, final walkthrough & settlement procedures",
      icon: <Calculator className="h-4 w-4" />,
      count: 0,
      href: "/family/home-buying/closing",
    },
  ]);
  ...
  const handleDocumentClick = (ev: MouseEvent) => {
    ...
    const clickedMenu = document.getElementById("homebuying-add-menu")?.contains(target);
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search home buying information…"
                value={query}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button
              ...
              aria-controls="homebuying-add-menu"
              data-testid="button-add-homebuying"
            >
              + Add Home Document
            </button>
            ...
            {addOpen && (
              ...
              >
                <div className="px-3 py-2 text-xs text-white/50">Add to Home Buying</div>
                <MenuItem icon={<DollarSign className="h-4 w-4" />} onClick={() => setAddOpen(false)}>
                  Financial Document
                </MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Property Listing</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Legal Document</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Professional Contact</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Inspection Report</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Closing Document</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Expense Record</MenuItem>
                <div className="px-3 py-2 text-xs text-white/50">Import</div>
                <MenuItem onClick={() => setAddOpen(false)}>Import documents…</MenuItem>
      ...
      <div className="mx-auto max-w-6xl px-6 py-8">
        <div className="mb-6">
          <p className="text-white/60 text-sm">
            Keep track of all aspects of home buying from financial planning to closing documentation.
          </p>
        </div>
        ...
        {totalItems === 0 && filtered.length > 0 && (
          <div className="mt-12 text-center" data-testid="getting-started">
            <HouseIcon className="h-16 w-16 text-white/40 mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-white mb-2">Organize Your Home Purchase</h3>
