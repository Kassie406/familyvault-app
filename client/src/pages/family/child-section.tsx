...
interface ChildSectionPageProps {
  title: string;
  createTo?: string;
}

export default function ChildSectionPage({ title, createTo }: ChildSectionPageProps) {
        ...
        <div className="mx-auto max-w-6xl px-6 py-4">
          <div className="flex items-center gap-4">
            <Button 
              variant="ghost" 
              size="sm" 
              onClick={() => setLocation('/family/child-information')}
        ...
        {data.length === 0 ? (
          <div className="mt-10 rounded-2xl border border-white/10 bg-white/[0.02] p-10 text-center">
            ...
            {createTo && (
              <Button 
                onClick={() => setLocation(createTo)}
                className="mt-4 bg-[#D4AF37] hover:bg-[#D4AF37]/80 text-black font-medium"
              >
