...
import { Plus, Search, ChevronRight, Plane, MapPin, FileText, Shield, Calendar, CreditCard, MoreHorizontal, Check, X, HelpCircle, Bell } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { useLocation } from "wouter";
import { LuxuryCard } from '@/components/luxury-cards';

type SectionKey = "documents" | "itinerary" | "emergency" | "destination" | "finances" | "health";
...
export default function InternationalTravel() {
  ...
  const [pageTitle, setPageTitle] = useState('International Travel');
  const [tempTitle, setTempTitle] = useState('International Travel');
  ...
  const [sections] = useState<Section[]>([
    {
      ...
      desc: "Passports, visas, travel insurance & identification documents",
      icon: <FileText className="h-4 w-4" />,
      count: 0,
      href: "/family/travel/documents",
    },
    {
      key: "itinerary",
      title: "Itinerary Planning",
      desc: "Flight details, accommodation bookings & travel schedules",
      icon: <Calendar className="h-4 w-4" />,
      count: 0,
      href: "/family/travel/itinerary",
    },
    {
      key: "emergency",
      title: "Emergency Information",
      desc: "Embassy contacts, emergency numbers & medical information",
      icon: <Shield className="h-4 w-4" />,
      count: 0,
      href: "/family/travel/emergency",
    },
    {
      key: "destination",
      title: "Destination Info",
      desc: "Local contacts, addresses & important location details",
      icon: <MapPin className="h-4 w-4" />,
      count: 0,
      href: "/family/travel/destination",
    },
    {
      key: "finances",
      title: "Travel Finances",
      desc: "Budget, expenses, currency & financial preparations",
      icon: <CreditCard className="h-4 w-4" />,
      count: 0,
      href: "/family/travel/finances",
    },
    {
      key: "health",
      title: "Health & Safety",
      desc: "Vaccinations, medical records & safety preparations",
      icon: <Shield className="h-4 w-4" />,
      count: 0,
      href: "/family/travel/health",
    },
  ]);
  ...
  const handleDocumentClick = (ev: MouseEvent) => {
    ...
    const clickedMenu = document.getElementById("travel-add-menu")?.contains(target);
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            ...
            {addOpen && (
              ...
              >
                <div className="px-3 py-2 text-xs text-white/50">Add to Travel Planning</div>
                <MenuItem icon={<FileText className="h-4 w-4" />} onClick={() => setAddOpen(false)}>
                  Travel Document
                </MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Flight Booking</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Hotel Reservation</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Emergency Contact</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Destination Info</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Travel Expense</MenuItem>
                <MenuItem onClick={() => setAddOpen(false)}>Health Record</MenuItem>
                <div className="px-3 py-2 text-xs text-white/50">Import</div>
                <MenuItem onClick={() => setAddOpen(false)}>Import itinerary…</MenuItem>
      ...
      <div className="mx-auto max-w-6xl px-6 py-8">
        <div className="mb-6">
          <p className="text-white/60 text-sm">
            Keep all travel documents, itineraries, and emergency information organized for safe international travel.
          </p>
        </div>
        ...
        {totalItems === 0 && filtered.length > 0 && (
          <div className="mt-12 text-center" data-testid="getting-started">
            <Plane className="h-16 w-16 text-white/40 mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-white mb-2">Organize Your International Travel</h3>
